...
import { whatWeBuild as whatWeBuildEs } from "@/lib/content.es";
import type { Locale } from "@/lib/i18n";

const copy = {
  en: { eyebrow: "What We Build", heading: "Everything a growing brand needs — under one roof." },
  es: {
    eyebrow: "Qué Construimos",
    heading: "Todo lo que una marca en crecimiento necesita — bajo un mismo techo.",
  },
};

export function WhatWeBuild({ locale = "en" }: { locale?: Locale } = {}) {
  const items = locale === "es" ? whatWeBuildEs : whatWeBuild;
  const t = copy[locale];
  return (
